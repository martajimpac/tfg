...
void main() {
  ...
  group('Evaluations Tests', () {
    ...
    Future<void> waitForEvaluationInserted(
      ...
      while (attempts < maxAttempts) {
        ...
        if (currentState is InsertarEvaluacionInicial) {
          ...
          fail("La evaluación falló con el error: ${currentState.errorMessage}");
        }
      ...
      fail("La evaluación no alcanzó un estado válido después de $maxAttempts intentos.");
    }
    ...
    Future<void> waitForEvaluacionFinished(
      ...
      fail("El pdf no se generó correctamente después de $maxAttempts intentos.");
    }
    ...
    Future<void> scrollToDatePicker(
      ...
      expect(pickerFinder, findsOneWidget, reason: 'No se encontró el date picker con la key $datePickerKey');
    ...
    Future<void> createEvaluation({
      ...
      await enterTextInFieldByHint(tester: tester, hintText: s.hintDenomination, value: denomination);
      await enterTextInFieldByHint(tester: tester, hintText: s.hintSerialNumber, value: serialNumber);

      // Rellenar las fechas
      final listViewFinder = find.byKey(listViewNewEvaluationKey);
      expect(listViewFinder, findsOneWidget, reason: "Main ListView not found on the screen.");

      await scrollToDatePicker(tester, fechaFabricacionKey, listViewFinder: listViewFinder);
      if(fechaFabricacion != null) {
        await selectDateFromPicker(tester, fechaFabricacionKey, fechaFabricacion);
      }

      await scrollToDatePicker(tester, fechaServicioKey, listViewFinder: listViewFinder);
    ...
    testWidgets('Filter evaluations by center', (tester) async {
      ...
      await selectCenter(tester, index: 1);
      await tester.pumpAndSettle();


      //Aplicar filtros
      final buttonApplyFilters = find.byKey(buttonApplyFiltersKey);
      expect(buttonApplyFilters, findsOneWidget, reason: "Apply filters button not found");
      await tester.tap(buttonApplyFilters);
      await tester.pumpAndSettle();

      //Comprobar que solo hay un resultado
      final listView = find.byKey(listViewEvaluationsKey);
      expect(listView, findsOneWidget, reason: "Evaluations ListView not found");
    ...
    testWidgets('Filter evaluations by date', (tester) async {
      ...
      await selectDateFromPicker(tester, filtroFechaRealizacionKey, DateTime(2025, 06, 15));
      await selectDateFromPicker(tester, filtroFechaCaducidadKey, DateTime(2026, 06, 16));
    ...
    testWidgets('Filter evaluations by denomination', (tester) async {
      ...
      await enterTextInFieldByHint(tester: tester, hintText: s.hintDenomination, value: "denominacion");
      await tester.pumpAndSettle();
    ...
    testWidgets('Create new evaluation', (tester) async {
      ...
      expect(buttonDialog, findsOneWidget, reason: "Confirm save button not found");
      await tester.tap(buttonDialog);
      await tester.pumpAndSettle();

      // Verificación del estado del cubit
      final evaluationsCubit = BlocProvider.of<InsertarEvaluacionCubit>(tester.element(find.byType(MaterialApp)));
      ...
      expect(listViewCheckList, findsOneWidget, reason: "Main ListView not found on the screen.");
      ...
      await tester.tap(find.byElementPredicate((element) => element == lastGestureDetector));
      await tester.pumpAndSettle();
      await tester.pumpAndSettle();
      ...
      await waitForEvaluacionFinished(tester, BlocProvider.of<PreguntasCubit>(tester.element(find.byType(MaterialApp))));
    });
    ...
    testWidgets('Create new evaluacion fails empty denomination', (tester) async {
      final s = S();
    ...
    testWidgets('Create new evaluacion fails empty serial number', (tester) async {
      final s = S();
    ...
    testWidgets('Create new evaluacion fails manufacture date after comissioning date', (tester) async {
      final s = S();
    ...
    testWidgets('Delete evaluation', (tester) async {
      ...
      final evaluacionesAntes = find.descendant(
        of: listView,
        matching: find.byType(GestureDetector), // O tu widget de item de evaluación
      );
      final totalAntes = evaluacionesAntes.evaluate().length;

      // 1. Buscar el WIDGET CONTENEDOR de la evaluación por nombre.
      final evaluacionFinder = find.widgetWithText(GestureDetector, 'Test Denomination');
      expect(evaluacionFinder, findsWidgets, reason: "Evaluation container with text 'Test Denomination' not found");
      ...
      expect(buttonDeleteInSpecificEvaluation, findsOneWidget, reason: "Delete button for 'Test Denomination' not found after long press");
      ...
      expect(buttonConfirmDelete, findsOneWidget, reason: "Confirm delete button not found");
